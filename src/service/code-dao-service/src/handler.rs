...
#[async_trait]
impl EventListenerAsyncRoutine<RouterHandlerPostObjectRequest, RouterHandlerPostObjectResult> for OnCommonPostHandle {
    async fn call(&self, param: &RouterHandlerPostObjectRequest) -> BuckyResult<RouterHandlerPostObjectResult> {
        ...
        if let Some(req_path) = req_path {
            ...
            if let Some("object") = req_path_type {
                ...
                if resp.is_err() {
                    error!("app req_path [{}] error: {:?}", req_path, resp.as_ref().err()); 
                }
	...
	let ctx = PostContext::new_by_param(param, self.stack.clone());
        info!("[{}] request route[{:?}], param:{:?}, caller:{:?}", ctx.id, ctx.route, ctx.data, ctx.caller);
        let ctx1  = ctx.clone();
