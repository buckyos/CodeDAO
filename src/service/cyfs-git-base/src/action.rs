...
pub async fn delete_object(stack: &Arc<SharedCyfsStack>, object_id: ObjectId) -> BuckyResult<()> {
    let _r = stack
        .non_service()
        .delete_object(NONDeleteObjectOutputRequest {
            common: NONOutputRequestCommon {
                req_path: None,
                source: None,
                dec_id: None,
...
pub async fn create_path_op_env(stack: &Arc<SharedCyfsStack>) -> BuckyResult<PathOpEnvStub> {
    ...
    let env = stack
        .root_state_stub(Some(ood), Some(dec_id()))
        .create_path_op_env()
        .await?;
    Ok(env)
}
    ...
    BC: Sync + Send + Clone + RawEncode + BodyContent,
{
    ...
    let req_path =
        Some(RequestGlobalStatePath::new(Some(service_dec_id()), req_path).format_string());
    ...
    BC: Sync + Send + Clone + RawEncode + BodyContent,
{
    ...
    info!(
        "Send sepcial object request ==> target OOD, req_path {:?}",
        req_path
    );
    let r = stack
        .non_service()
...
pub async fn file_task(
    stack: &Arc<SharedCyfsStack>,
    task_file_id: ObjectId,
    local_path: PathBuf,
    from_device_id: DeviceId,
    file_object_target: Option<ObjectId>,
) -> BuckyResult<()> {
