...
impl  ConfigManager {
    ...
    pub fn new_with_content(content: &str) -> Self {
        ...
        let app_service_dec_id = DecApp::generate_id(owner, CYFS_GIT_SERVICE_NAME);
	...
	let content = std::fs::read_to_string("config/config.toml")
            .expect("config config.toml file no ok");
        let root:ConfigRoot = toml::from_str(&content).unwrap();
...
pub async fn set_current_space(stack: &Arc<SharedCyfsStack>) ->BuckyResult<()> {
    ...
    CURRET_SPACE.set(user.name().to_string())
        .map_err(|e| {
            error!("set_current_space: {:?}", e);
            BuckyError::new(BuckyErrorCode::InternalError, format!("set_current_space error:{}", e))
        })?;
    Ok(())
}
...
pub async fn is_ood_service(stack: &Arc<SharedCyfsStack>) {
    ...
    if info.info.is_ood_device && info.info.ood_device_id.to_string() == service_ood_device.to_string() {
