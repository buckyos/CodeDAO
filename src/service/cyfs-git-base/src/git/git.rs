...
pub fn git_show_target_branch_ref(repo_dir: PathBuf, branch: &str) -> BuckyResult<String> {
    ...
    let stdout = git_exec_base(repo_dir, ["show-ref", "--heads", "-s", branch])?;
    let hash = stdout.lines().next().map(|x| x.to_owned()).unwrap();
...
pub fn git_rev_parse(repo_dir: PathBuf, rev: &str) -> BuckyResult<String> {
    let result = git_exec_base(repo_dir, ["rev-parse", rev])?;
...
pub fn git_unpack_objects(repo_dir: PathBuf, pack_file_path: PathBuf) {
    ...
    let mut command = git_exec_base_command(repo_dir.clone(), ["unpack-objects"]);
    let process = command
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
...
pub fn git_unbundle_and_unpack_objects(
    repo_dir: PathBuf,
    bundle_file: &str,
    branch: &str,
    ref_hash: &str,
) -> BuckyResult<()> {
    let _stdout = git_exec_base(repo_dir.clone(), ["fetch", bundle_file, branch])?;

    git_update_ref(repo_dir, branch, ref_hash);
    Ok(())
}

pub fn git_bundle_create(
    repo_dir: PathBuf,
    bundle_target: &str,
    git_rev_list_arg: &str,
) -> BuckyResult<()> {
...
/// git_ls_tree_file
/// 查找blob文件对应的hash id
pub fn git_ls_tree_file(
    repo_dir: PathBuf,
    branch: &str,
    file_path: &str,
) -> BuckyResult<GitLsTreeResultTreeData> {
...
/// 文件内容
pub fn git_show_file_content(
    repo_dir: PathBuf,
    blob: &str,
) -> BuckyResult<Vec<GitFileLineContent>> {
...
pub fn git_log_commit_message(
    repo_dir: PathBuf,
    branch: &str,
    file_path: &str,
) -> BuckyResult<GitFileCommitMessage> {
    info!(
        "git_log_commit_message params {:?} , file_path: {:?}",
        branch, file_path
    );
    ...
    Err(BuckyError::new(
        BuckyErrorCode::Failed,
        "git log commit message error",
    ))
}

/// git_log_last_commit_message
/// 最后一次commit的信息
pub fn git_log_last_commit_message(
    repo_dir: PathBuf,
    branch: &str,
) -> BuckyResult<GitFileCommitMessage> {
    let stdout = git_exec_base(
...
pub fn git_commits(repo_dir: PathBuf, branch: &str) -> BuckyResult<Vec<GitCommit>> {
    ...
    let commits: Vec<GitCommit> = result
        .lines()
        .map(|log| {
            debug!("commit oid {}", log);
            let oid = log.trim().trim_matches('\'');
            let commit =
                git_read_commit_object(repo_dir.clone(), oid).expect("parse commit info failed");
            commit
        })
        .collect();
    debug!(
        "git read commits of branch {}, count {} ",
        branch,
        commits.len()
    );
    Ok(commits)
...
fn get_diff_line(input: &str) -> BuckyResult<GitDiffLineCount> {
    // "@@ -3,4 +3,4 @@"
    let re = Regex::new(
        r"@@[ ][\+\-](?P<left>\d+)(,(?P<left2>\d+))?[ ][\+\-](?P<right>\d+)(,(?P<right2>\d+))?[ ]",
    )
    .unwrap();
    let caps = re.captures(input);
    if caps.is_none() {
...
impl CommitDiffResultLine {
    ...
    fn code_line(content: &str, line_result: &mut GitDiffLineCount) -> CommitDiffResultLine {
        ...
        if line_result.left_count < line_result.left_totle + line_result.left
            && result.diff_type != "add"
        {
        ...
        if (line_result.right_count < line_result.right_totle + line_result.right
            && result.diff_type != "remove")
            || is_left_zero
        {
...
/// diff 信息 结构化
pub fn git_show_commit_diff(
    repo_dir: PathBuf,
    parent: &str,
    commit_id: &str,
) -> BuckyResult<Vec<CommitDiffResult>> {
...
pub fn git_compare_no_merge(
    ...
    let stdout = git_exec_base(
        repo_dir,
        ["log", &compare_param, "--oneline", "--no-merges"],
    )
    .unwrap();
...
pub fn git_merge_diff_file(
    repo_dir: PathBuf,
    origin_branch: &str,
    target_branch: &str,
    file_name: &str,
) -> BuckyResult<Vec<CommitDiffResultLine>> {
    let compare_param = format!("{}..{}", target_branch, origin_branch);
...
mod main_tests {
    ...
    async fn test_reg() {
        let input =
            "diff --git a/services/cyfs-git-base/src/git.rs b/services/cyfs-git-base/src/git.rs";
    ...
    async fn test_git_rev_list_objects() {
        ...
        let result =
            git_rev_list_objects(repo_dir, "64f645d294f3916b25588f20a7ce24df45720739").unwrap();
        println!("result {:?}", result);
    }
    ...
    async fn test_git_unpack_objects() {
        ...
        let pack_file_path = PathBuf::from(
            r"D:\app\clone\test\.git\objects\pack\pack-0a126038e26417b7692bd3c0c275376e29148346.pack",
        );
        let result = git_unpack_objects(pack_file_path, repo_dir);
        println!("result {:?}", result);
    }
