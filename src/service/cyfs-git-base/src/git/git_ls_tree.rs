...
pub fn git_ls_object_parse(input: &str)-> BuckyResult<GitLsTreeResultTreeData> {
    ...
    if result.is_none() {
        ...
        return Err(BuckyError::new(BuckyErrorCode::Failed, "parse ls-tree result error"))
    }
...
pub fn git_ls_tree_path(repo_dir: PathBuf, branch: &str, file_path: &str) -> BuckyResult<GitLsTreeResult> {
    ...
    if stdout.is_ok() {
        ...
        let tree:Vec<GitLsTreeResultTreeData> = stdout.lines().map(|line| {
            git_ls_object_parse(line).unwrap()
        }).collect();
    ...
    } else { // ls-tree 命令出错了，那个这个hash就是个非tree对象，目前先当文件blob类型处理
        Ok(GitLsTreeResult{
...
pub fn stat_repo_with_cache(repo_dir: PathBuf, branch: &str, cache_key: &str) -> BuckyResult<HashMap<LanguageType, usize>>{
    ...
    if cache_value.is_some() {
        ...
        info!("get languages cache: {} , value: {}",cache_key, cache_value);
        let languages:HashMap<LanguageType, usize> =  serde_json::from_str(&cache_value).map_err(transform_err)?;
        return Ok(languages)
    } else {
        ...
        let languages:HashMap<LanguageType, usize> = serde_json::from_str(&value).map_err(transform_err)?;
        return Ok(languages)
    }
}

pub fn stat_repo(repo_dir: PathBuf, branch: &str) -> BuckyResult<Arc<Mutex<HashMap<LanguageType, usize>>>> {
...
pub fn stat_file(file_name: &str, file_hash: &str, config: &Config, repo_dir: Arc<PathBuf>, languages: Arc<Mutex<HashMap<LanguageType, usize>>>) {
    ...
    info!("[{}] {:?} code_state {:?}. thread: {:?}",file_name, language_type, code_state, thread::current().id());
