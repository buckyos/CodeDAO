...
pub fn tree_map_path(author_name: &str, repo_name: &str, tree_id: &str) -> String {
    format!("{}{}/{}/tree/{}", REPOSITORY_PATH, author_name, repo_name, tree_id)
}


#[async_trait]
pub trait TreeUtil {
    async fn insert_tree(&self, stack: &Arc<SharedCyfsStack>, space:&str, repo_name: &str ) -> BuckyResult<()>;
    async fn get_tree_object(stack: &Arc<SharedCyfsStack>, space:&str, repo_name: &str, tree_oid: &str) -> BuckyResult<Tree>;
    async fn get_tree_from_path(&self, stack: &Arc<SharedCyfsStack>, space:&str, repo_name: &str, sub_dir: &str) -> BuckyResult<Tree>;
    fn read_tree_file_map(&self) -> BuckyResult<HashMap<String, (String, String)>> ;
    fn get_tree_from_git(owner: ObjectId, space:&str, repo_name: &str, tree_oid: &str) ->BuckyResult<Tree>;
}
...
impl TreeUtil for Tree {
    async fn insert_tree(&self, stack: &Arc<SharedCyfsStack>, space:&str, repo_name: &str) -> BuckyResult<()> {
        ...
        if result.is_some() {
            info!("repository tree object[{}] alread in objectmap", self.tree_id());
            return Ok(())
        }
        let _r = env.set_with_path(full_path.clone(), &id, None, true).await?;
        let _r = env.commit().await?;
        info!("insert repository tree object ok {}: {}", full_path, self.tree_id());
        Ok(())
    }
    ...
    async fn get_tree_object(stack: &Arc<SharedCyfsStack>, space:&str, repo_name: &str, tree_oid: &str) -> BuckyResult<Tree> {
        let env = create_path_op_env(stack).await?;
        let full_path = tree_map_path(space, repo_name, &tree_oid);
        let object_id = env.get_by_path(full_path).await?;
        if object_id.is_none() {
            return Err(BuckyError::new(BuckyErrorCode::NotFound, "not found tree object id"))
        }
    ...
    // recurse
    async fn get_tree_from_path(&self, stack: &Arc<SharedCyfsStack>, space:&str, repo_name: &str, sub_dir: &str) -> BuckyResult<Tree>{
        let target = self.tree()
            .into_iter()
            .find(|x| sub_dir.to_string() == x.file_name);
        if target.is_none() {
            return Err(BuckyError::new(BuckyErrorCode::NotFound, "not found target dir's tree"))
        }
...
pub async fn commits_root_tree_object_loop_insert_map(stack: &Arc<SharedCyfsStack>, owner: ObjectId, repo_dir: PathBuf, top_commit_id: &str, space: &str, repo_name: &str)-> BuckyResult<()> {
    ...
    loop {
        ...
        if current_commit.parent != "" {
            loop_stack.push(
                Arc::new(git_read_commit_object(repo_dir.clone(), &current_commit.parent)?)
            );
        }
        if current_commit.parent2 != "" {
            loop_stack.push(
                Arc::new(git_read_commit_object(repo_dir.clone(), &current_commit.parent2)?)
            );
        }

        // info!("commit {:?},  poid {:?}", commit.object_id, commit.parent);
        root_tree_object_loop_insert_map(stack, owner, repo_dir.clone(), &current_commit.tree, space, repo_name).await?;
        commit_count += 1
    }
...
pub async fn root_tree_object_loop_insert_map(stack: &Arc<SharedCyfsStack>, owner: ObjectId, repo_dir: PathBuf, root_tree_id: &str, space: &str, repo_name: &str) -> BuckyResult<()> {
...
impl TreeFileHelper {
    ...
    pub async fn target_tree_file(&self, commit: Arc<GitCommit>) -> BuckyResult<HashMap<String, (String, String)>> {
        let mut it = std::path::Path::new(&self.sub_dir).iter();

        let mut tree =  {
            let result = Tree::get_tree_object(&self.stack, &self.space, &self.repo_name, &commit.tree).await;
            if result.is_ok() {
                result.unwrap()
            } else {
                let owner = get_owner(&self.stack).await;
                let tree_object = Tree::get_tree_from_git(owner, &self.space, &self.repo_name, &commit.tree)?;
                tree_object
            }
        };
        ...
        while let Some(target_path_step) = it.next() {
            ...
            tree = tree.get_tree_from_path(
                &self.stack, &self.space, &self.repo_name, target_path_step).await?;
        }
    ...
    // match handler
    pub fn file_match<S: Into<String>>(&mut self, key: S, commit: Arc<GitCommit>, file_type: String) {
        let key = key.into();
    ...
    fn relationship(&mut self, key: String, children_oid: String) {
        if self.mark_commit_relationship.contains_key(&key) { // update
            ...
            self.mark_commit_relationship.insert(
                key.to_string(), 
                Arc::new(Mutex::new(vec![children_oid]))
            );
        }
    }
    ...
    pub async fn start(&mut self) -> BuckyResult<HashMap<String, TreeFileMessage2>> {
        ...
        loop {
            ...
            if file_map.is_err() {
                info!("no tree file , set child commit oid and info [{}]", current_commit_oid);
            ...
            for (file_name, value) in self.init_file_map.clone().into_iter() {       
                if file_map.get(&file_name).is_none() { // new file. write result map
                    info!("tree no file [{}] , set child commit oid [{}]",file_name, current_commit_oid);
                    let target_commit = self.child(&current_commit_oid);
                    self.file_match(file_name, target_commit, value.1.clone());
                } else {
                    // arr:[hash,file_type]
                    ...
                    if prev_file_hash != current_file_hash {
                        info!("file[{}] in tree and modify  , set child commit oid [{}]",file_name, current_commit_oid);
                        let target_commit = self.child(&current_commit_oid);
                        self.file_match(file_name, target_commit, value.1.clone());
                    } else if is_bottom && prev_file_hash == current_file_hash {
                        info!("no file[{}] and init commit  , set current commit oid [{}]",file_name, current_commit_oid);
...
public_service_ood="5aSixgM1oBicrsUdS3nyKM1MA9AgiMEE2y2qFQ3jTTYB""#,
    ...
    async fn repository_read_and_write_tree_object()  ->  BuckyResult<()> {
        ...
        assert_eq!(tree_object2.tree_id().to_string(), tree_object.tree_id().to_string());
    ...
    async fn test_commits_root_tree_object_loop_insert_map()  ->  BuckyResult<()> {
        ...
        commits_root_tree_object_loop_insert_map(&stack, owner, path.clone(), commit_oid, SPACE, REPO_NAME).await?;
    ...
    async fn test_read_repository_home_files_by_markup_clean()  ->  BuckyResult<()> {
        ...
        for (key, value) in  result_main_file_map.clone().into_iter() {
            info!("[{}] ({})  {}  {}", key, value.file_type, value.message, value.hash);
        }
